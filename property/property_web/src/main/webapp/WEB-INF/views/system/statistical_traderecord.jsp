...
<body class="p-statistical p-statistical-traderecord">
<div class="wrapper">
                ...
                <div class="col-md-12" role="main">
                    ...
                    <c:choose>
                    	<c:when test="${ifcheck==1}">
	                    	<span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=1'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:2px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每日)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=2'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每周)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=3'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当年每月)</span>
                    	</c:when>
                    	<c:when test="${ifcheck==2}">
                    		<span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=1'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每日)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=2'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:2px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每周)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=3'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当年每月)</span>
                    	</c:when>
                    	<c:when test="${ifcheck==3}">
                    		<span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=1'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每日)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=2'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:0px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当月每周)</span>
		                    <span onclick="location.href='<%=request.getContextPath() %>/system/traderecordlist?ifcheck=3'" style="display:inline-block;padding:0 8px 4px 3px;height:26px;border-bottom:2px solid #1badf2;text-decoration: none;width: 210px;text-align: center;cursor: pointer;">交易金额统计(当年每月)</span>
                    	</c:when>
                    </c:choose>
                    </h3>
<%--                     <div class="row area-list-dec">
                        <div class="area-search">
                            <form class="form-inline" method="post" action="<%=request.getContextPath()%>/system/traderecordlist">
                            	<input type="hidden" name="ifcheck" value="${ifcheck}">
                                	代理商：<select name="agencyid" onchange="AgencyChange(this)"  class="form-control select2-input  js-states select2-hidden-accessible select2-offscreen js-example-basic-multiple" multiple="multiple" style="width:200px" value="">
                                 	<c:forEach items="${agencylist}" var="item">
                                 		<c:choose> 
                                 			<c:when test="${map.agencyid==item.userid}">
                                 				<option value="${item.userid}" selected="selected">${item.agencyname}</option>
                                 			</c:when>
                                 			<c:otherwise>
                                 				<option value="${item.userid}">${item.agencyname}</option>
                                 			</c:otherwise>
                                 		</c:choose>
                                 	</c:forEach>
                                	 </select> 
                                	销售：<select name="salerid" class="form-control select2-input  js-states select2-hidden-accessible select2-offscreen js-example-basic-multiple" multiple="multiple" style="width:200px">
                                 	<c:forEach items="${salerlist}" var="item">
                                 		<c:choose>
                                 			<c:when test="${map.salerid==item.userid}">
                                 				<option value="${item.userid}" selected="selected">${item.agencyname}</option>
                                 			</c:when>
                                 			<c:otherwise>
                                 				<option value="${item.userid}">${item.agencyname}</option>
                     ...
                     <div class="row area-list-dec">
                     <div class="area-search">
                            <form class="form-inline" method="post" role="form" action="<%=request.getContextPath()%>/system/traderecordlist?ifcheck=${ifcheck}">
                                <span>分销渠道：</span>
...
var setting = {
	...
	dataArray.push(obj={
	 ...
	 <c:choose>
	  <c:when test="${agenList.size()>0}">
	  <c:forEach items="${agenList}" var ="row">
	       var  obj ={
	    	userid:'${row.userid}',
	        id:'${row.agencyid}',
	        pId:'${row.parentid}',
...
require(
  ...
  <form action="<%=request.getContextPath() %>/system/clicktraderecord" id="v_form" method="post">
