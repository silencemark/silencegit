...
<div class="page_selbox">
	<span id="time_span" style="width:30%">时间</span>
...
<%--     <span id="salary_span">薪资</span>
    ...
    <span style="background: #F1F1F1;width:30%;" id="jobtypename" onclick="changejobtype()">
    	<c:choose>
...
<%-- 		<c:forEach items="${projectlist}" var="item">
	    	<li onclick="location.href='<%=request.getContextPath()%>/workers/initgrabproject?projectid=${item.projectid}'">
	        	<div class="user_img">
	            	<b><img src="${item.headimage}" width="62" height="62"></b>
	                <span class="name_v">${item.contacter}</span>
	            </div>
	            <div class="user_box p_right">
	                <div class="xx_01 word_hidden">${item.projecttitle}<i id="space">${item.spacing}</i></div>
	                <div class="xx_02 word_hidden">${item.companyname}</div>
	                <div class="xx_03 word_hidden"></div>
	            </div>
	            <div class="ico_box">
	            	<span class="time">项目周期：<fmt:formatDate value="${item.starttime}" pattern="yyyy-MM-dd"/>  至 <fmt:formatDate value="${item.endtime}" pattern="yyyy-MM-dd"/> </span>
...
function getFristInfo(){
	...
	data1.pageNo = 1;
	$.ajax({
		url:"<%=request.getContextPath() %>/workers/projectlistAjax",
		data:data1,
		success:function(data){
			...
			for(var i=0;datalist.length>i;i++){
				var d = datalist[i]; 
			     ...
			     if(d.endtime !=null){
				  ...
				  $("#project_ul").append(hhutil.replace(html,datalist[i])).trigger("create");
			}
...
<script type="text/temple" id="tmpl_li">
 		<li>
                ...
                <div onclick="location.href='<%=request.getContextPath()%>/workers/initgrabproject?projectid={projectid}'">
	            <div class="user_box p_right">
	                <div class="xx_01 word_hidden">{projecttitle}<i id="space">{spacing}</i></div>
...
<form action="<%=request.getContextPath() %>/workers/projectlist" method="post" id="screenForm">
