 ...
 <script src="http://api.map.baidu.com/api?v=2.0&ak=wBYHoaC0rzxp8zqGCdx9WXxa" type="text/javascript"></script>  
  <script type="text/javascript">
	...
	function getbaidumap(){
		<c:if test="${projectmap.longitude == null}">
		...
		<c:if test="${projectmap.longitude != null}">
			longitude = '${projectmap.longitude}';
			latitude = '${projectmap.latitude}';
		initialize();
		</c:if>
		loadingbody();
	}
      ...
      //创建图标对象  
        ...
        function initialize(){
            ...
            <c:if test="${order.longitude != null}">
            ...
            map.addControl(new BMap.MapTypeControl());  
	            ...
	            <c:if test="${order.applicantid==createrid}">
	            	black="<a style='float:right' href=\"<%=request.getContextPath()%>/my/edit\"><img src=\"../upload/black.jpg\" width=\"14px\" height=\"22px\"/></a>";
	            	nickname="我(${order.nickname})";
	            </c:if>
                var opts={
                        width:200,//信息窗口宽度height:100,//信息窗口高度  
                        height:80,
        ...
        function loadingbody(){
        	$.ajax({
        		type:'post',
        		dataType:'json',
        		url:'<%=request.getContextPath()%>/employer/getnoticenum?orderid=${projectmap.orderid}',
       ...
       function getnoticeandtime(){
        	...
        	{
        	var now = new Date(); 
        	var leftTime=parseInt('${projectmap.starttimesecond}')-now.getTime();
        	if(leftTime <= 0){
        		$('#timestr').text("已过期");
        	}else{
...
<c:if test="${isWeiXinFrom==false}">
	 <div class="headbox">
		<div class="title f-20">项目地图明细</div>
	     <a href="javascript:turnBack();" class="ico_back f-16">返回</a>
    <!-- <a href="#" class="ico_list">更多</a> -->
	</div>
</c:if>
<div class="pagelist_nav">
   <a href="<%=request.getContextPath() %>/employer/releasemapinfo?projectid=${projectmap.projectid}&orderid=${projectmap.orderid}" id="ing"  class="line active">项目申请列表</a>
   <a href="<%=request.getContextPath() %>/order/getEmployerOrderProjectDetail?projectid=${projectmap.projectid}&orderid=${projectmap.orderid}" id="end" >项目基本信息</a></div>
...
<div class="order_list01">
	...
	<ul>
        ...
        <li>
        	<div class="user_img" style="width: auto" onclick="javascript:window.location.href='<%=request.getContextPath()%>/my/goWorkmandetail?orderid=${row.orderid }&userid=${row.applicantid }&applyorderid=${row.applyorderid }&projectid=${map.projectid }'">
            	<b><img src="${row.headimage }" width="62" height="62"></b>
                <span class="name" style="width:62px;">${row.nickname}</span>
                <c:if test="${row.isattention==1}">
            ...
            <div class="user_box2" onclick="javascript:window.location.href='<%=request.getContextPath()%>/my/goWorkmandetail?orderid=${row.orderid }&userid=${row.applicantid }&applyorderid=${row.applyorderid }&projectid=${map.projectid }'">
         ...
         <%--     <div class="tool_xx"  style ="width: 94px">
                 <c:if test="${row.status == 3 }"><div class="state bg_yellow" onclick="cancelOrder('${row.orderid }','${row.applyorderid}');">取消</div></c:if>
            	 <div class="zx_xx01">
                   <c:choose>
                   ...
                   <c:if test="${row.status == 3 }">
                     <c:choose>
                      <c:when test="${row.score == null and row.status_order == 2 }"> 
                        <div style="margin-top:4px" onclick="javascript:window.location.href='<%=request.getContextPath() %>/evaluate/initAddEvalue?orderid=${row.applyorderid }&evaluationerid=${row.publisherid }&userid=${row.applicantid }&projectid=${projectmap.projectid }&oid=${row.orderid }'"  class="state bg_yellow">评价</div>
                      </c:when>
...
<script type="text/temple" id="tmpl_li">
  <li>
        	<div class="user_img" style="width: auto" onclick="javascript:window.location.href='<%=request.getContextPath()%>/my/goWorkmandetail?orderid={orderid}&userid={applicantid }applyorderid={applyorderid }&projectid={projectid}'">
            ...
            <div class="user_box2" onclick="javascript:window.location.href='<%=request.getContextPath()%>/my/goWorkmandetail?orderid={orderid}&userid={applicantid }applyorderid={applyorderid }&projectid={projectid}'">
