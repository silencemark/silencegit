...
<!--头部-->
<div class="headbox">
  <div class="title f-20">手机快捷登陆</div>
...
<div class="body_minheight">
	<div class="login">
		<ul>
	    	<li class="line"><span>手机号</span><input type="text" class="text" name="username" maxlength="11" placeholder="请输入手机号"></li>
	<%--     	<li class="yzm_box"><span></span><img id="vcode2" style="margin-left: -50px;" src="<%=request.getContextPath() %>/vcode/getestVcode?"+new Date().getTime() onclick='this.src="<%=request.getContextPath() %>/vcode/getestVcode?"+new Date().getTime()' width="76" height="32" /></li>
	    	<li class="yzm_box"><span>图型码</span><input type="text" name="userCode" id="nowcode" value="" placeholder="请输入图形验证码"/></li>
	         --%><li class="yzm_box"><span>验证码</span><input type="text" class="text" id="code" placeholder="请输入手机验证码"><input type="button" class="yzm_btn"  value="获取验证码"></li> 
	    </ul>
	</div>

<div class="main_bigbtn"><input type="button" onclick="register();" value="验证并登陆"></div>
<div class="login_link"><a href="<%=request.getContextPath()%>/login/inintLoginPassword">密码登陆</a></div>
...
<%-- 		  $.ajax({
			  type:"post",
			  url:"<%=request.getContextPath()%>/system/gettestcode",
			  success:function(data){
				  if(data !="" && data != null){
					  var nowcode=$('#nowcode').val();
					  if(data.toLowerCase() == nowcode.toLowerCase()){
						  xians(60,$(this)); 
						  getCode($("input[name='username']").val());
					  }
					  else{
						 alert("请正确输入图形验证码");
					  }
  ...
  function getCode(phone){
		  ...
		  $.ajax({
			    type:"post",
			    url:"<%=request.getContextPath()%>/login/sendPhoneCode/"+phone,
			    success:function(data){
  ...
  function register(){
	  ...
	  $.ajax({
		    type:"post",
		    url:"<%=request.getContextPath()%>/login/validateCode",
		    data:opt,
		    success:function(data){
		    	if(data.flg){
