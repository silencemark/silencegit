...
<div class="order_detail">
	<ul>
        <li><span>订单标题：</span><i>${applydetailmap.jobtitle}</i><div class="clear"></div></li>
        <li><span>订单单号：</span><i>${applydetailmap.orderno}</i><div class="clear"></div></li>
        <li><span>工种：</span><i>${applydetailmap.jobtype}</i><div class="clear"></div></li>
        <li><span>薪资(元/人/天)：</span><i style="width:50%;">${applydetailmap.salary}元</i><div class="clear"></div></li>
        <li><span>结算方式：</span><i>${applydetailmap.settlementmethod}</i><div class="clear"></div></li>
        <li><span>用工时间(开始)：</span><i style="width:50%"><fmt:formatDate value="${applydetailmap.starttime}" pattern="yyyy-MM-dd HH:mm"/></i><div class="clear"></div></li>
        <li><span>用工时间(结束)：</span><i style="width:50%"><fmt:formatDate value="${applydetailmap.endtime}" pattern="yyyy-MM-dd HH:mm"/></i><div class="clear"></div></li>
        <li><span>工作地区：</span><i>${applydetailmap.provincename} ${applydetailmap.cityname}</i><div class="clear"></div></li>
        <li><span>工作地址：</span><i>${applydetailmap.workplace}</i><div class="clear"></div></li>
        <li><span>联系人：</span><i>${applydetailmap.contacter}</i><div class="clear"></div></li>
        <li><span>联系方式：</span><i onclick="tel('${applydetailmap.telephone}');">
               <c:choose>
                 <c:when test="${applydetailmap.ifpay eq 1 }">${applydetailmap.telephone}</c:when>
                 <c:when test="${applydetailmap.ifpay eq 0 }"> ${fn:replace(applydetailmap.telephone, fn:substring(applydetailmap.telephone,3,7), '****')}</c:when>
               </c:choose>
            
            </i><div class="clear"></div></li>
        <li><span>公司名称：</span><i>${applydetailmap.companyname}</i><div class="clear"></div></li>
        <li><span>工作要求：</span><i>${applydetailmap.jobrequirements}</i><div class="clear"></div></li>
        <li class="last"><span>订单状态：</span><i class="yellow">
             <c:choose>
...
wx.ready(function(){
	...
	//title:分享的标题；link分享的链接；imgUrl分享的图标
 	wx.onMenuShareAppMessage({
 	    title: '嘀嗒叫人',
 	    desc: context,
 	    link:  "<%=Constants.PROJECT_PATH%>employer/applyJobDetail?applyorderid=${applydetailmap.applyorderid}",
 	    imgUrl:  "<%=Constants.PROJECT_PATH%>upload/didalogo.png",
 	    success: function () {
 	    	$.ajax({
 	    		type:'post',
 	    		dataType:'json',
 	    		url:'<%=request.getContextPath()%>/employer/givesharecoin?pagename=抢单详情&sharesourceid=${applydetailmap.applyorderid}',
 	    		success:function(){
 	    		}
 	    	})
 	    }
