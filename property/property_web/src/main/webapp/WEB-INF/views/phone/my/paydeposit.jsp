...
function callpayback(data){
	if(data.success==1){
		swal({
				...
				window.location.href="<%=request.getContextPath()%>/my/paysuccess?type=2&out_trade_no=${appSignMap.out_trade_no}";
			});
	}else{
		swal("","支付失败！");
	}
		...
		//   	 				alert(res.err_code + res.err_desc + res.err_msg);
	    		...
	    		if($('#tickcoin').val() > 0){
	    			swal({
	    				...
	    				window.location.href="<%=request.getContextPath()%>/my/paysuccess?type=1&out_trade_no=${appSignMap.out_trade_no}";
	    			});
	    		}else{
