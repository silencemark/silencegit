$(function(){
	...
	var keyword = {
		init: function(){
			...
			$("tbody").on("click", ".audit_yes", keyword.auditPass);
			$("tbody").on("click", ".audit_no", keyword.auditRefuse);
		},
		auditPass: function(){
			//审核通过
			var id = $(this).data("id");
		...
		auditRefuse : function(){
			...
			$.alert({
				title: "温馨提示",
				txt: "<p>拒绝原因</p><select id='refusereason' style='display:block;' class='input'>"+
					...
					"</select>"+
					"<p>其他原因</p><input type='text' class='input' id='otherreason'/>",
				btnY: "确定",
				btnN: "取消",
