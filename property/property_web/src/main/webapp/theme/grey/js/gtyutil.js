									...
									for(var j=0,lenj=itemList.length;j<lenj;j++){
										...
										if(!hhutil.isEmpty(item)){
											if(j==0){
												...
												html += "<div class='sub clearFix'><p><a target='_blank' href='"+hhutil.getRootPath()+"/ws/backer/wxset/showDTImgText.action?imgtextlistid="+item.imgtextlistid+"'>"+hhutil.getValue(item.title)+"</a></p><div class='img'><img src='"+hhutil.getRootPath()+hhutil.getValue(item.imgurl)+"'/></div></div>";
											}
										}
									}
								}
