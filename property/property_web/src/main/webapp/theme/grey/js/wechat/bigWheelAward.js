$(function(){
	...
	var keyword = {
		...
		showPop : function(o) {
			if (!o) {
				o = {awardname:"",wintip:"",awardpercent:"",awardtotalpcount:"",
			...
			$.alert({
				...
				callbackY: function(){
					...
					//验证中奖名额
					if(awardtotalpcount.inputEmpty()){
						awardtotalpcount.inputError("中奖名额不能为空");
						return false;
					}else if (!hhutil.checkInt(awardtotalpcount.val())){
						awardtotalpcount.inputError("中奖名额只能为数字");
						return false;
					}
					...
					var alinputs = "<input type='hidden' name='wheelAwardList.awardname' class='awardname' value='" + awardname.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.wintip' class='wintip' value='" + wintip.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.awardpercent' class='awardpercent' value='" + awardpercent.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.awardtotalpcount' class='awardtotalpcount' value='" + awardtotalpcount.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.validtype' class='validtype' value='" + validtype.attr("value") + "' />"
						+ "<input type='hidden' name='wheelAwardList.validdaynum' class='validdaynum' value='" + validdaynum.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.validendtime' class='validendtime' value='" + validendtime.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.desc' class='desc' value='" + desc.val() + "' />"
						+ "<input type='hidden' name='wheelAwardList.id' class='id' value='" + id.val() + "' />";
				...
				callbackY: function(){
					...
					setTimeout(function(){
						...
						if(!hhutil.isEmpty(id)){
							//添加删除的状态
							tr.find(".console").append("<input type='hidden' name='wheelAwardList.state' value='1'/>");
							tr.hide();
						}else{
							tr.remove();
						}
